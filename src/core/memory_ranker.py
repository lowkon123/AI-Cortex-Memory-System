...
class MemoryRanker:
    ...
    def __init__(self, weights: Optional[dict[str, float]] = None):
        """Initialize the ranker with custom weight factors."""
    ...
    def score_memory(
        ...
        """Calculate the overall cognitive score for a single memory node."""
    ...
    def explain_score(
        ...
        """Calculate the score and provide a breakdown of how it was derived."""
    ...
    def rank_memories(
        ...
        """Rank and filter a list of memories, applying redundancy reduction (novelty penalty)."""
        ...
        # Apply Novelty Penalty (Iterative Selection)
        selected_nodes = []
    ...
    def _score_similarity(
        ...
        """Handle cosine similarity calculation between query and memory vectors."""
    ...
    def _score_recency(self, memory: MemoryNode, now: datetime) -> float:
        """Score based on exponential decay from the last modification or access time."""
    ...
    def _score_emotion(self, memory: MemoryNode) -> float:
        """Apply a small bonus for emotional or strongly valenced memories."""
        sentiment_bonus = {
            "mixed": 0.12,
            "positive": 0.05,
            "negative": 0.05,
    ...
    def _score_confidence(self, memory: MemoryNode) -> float:
        """Score based on the source or factual confidence of the memory."""
        return memory.confidence

    def _score_persona(self, memory: MemoryNode, persona: Optional[str]) -> float:
        """Boost memories that match the current active persona context."""
        if not persona:
            return 0.5
        return 1.0 if memory.persona == persona else 0.15

    def _score_concepts(self, memory: MemoryNode, query_tags: list[str]) -> float:
        """Calculate conceptual overlap between query tags and memory concept tags."""
    ...
    def _score_kind(
        ...
        """Score based on the memory type (Working, Episodic, Semantic)."""
